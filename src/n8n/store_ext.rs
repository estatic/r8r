...
/// A nullable text column. (sqlx's `Any` driver refuses to decode a
/// PostgreSQL NULL into `Option<T>` through the checked getter.)
pub(super) fn opt_text(r: &AnyRow, column: &str) -> Option<String> {
    r.try_get_unchecked::<Option<String>, _>(column).ok().flatten()
}

pub(super) fn opt_int(r: &AnyRow, column: &str) -> Option<i64> {
    r.try_get_unchecked::<Option<i64>, _>(column).ok().flatten()
}

fn user_from(r: &AnyRow) -> User {
    User {
        id: r.get("id"),
        email: r.get("email"),
        first_name: opt_text(r, "first_name"),
        last_name: opt_text(r, "last_name"),
        password: opt_text(r, "password"),
        role: r.get("role"),
...
fn execution_from(r: &AnyRow) -> ExecutionRow {
    let parse = |s: Option<String>| s.and_then(|s| serde_json::from_str::<Value>(&s).ok());
    ExecutionRow {
        id: r.get("id"),
        workflow_id: opt_text(r, "workflow_id"),
        mode: r.get("mode"),
        status: r.get("status"),
        finished: r.get::<i64, _>("finished") != 0,
        retry_of: opt_text(r, "retry_of"),
        started_at: r.get("started_at"),
        stopped_at: opt_text(r, "stopped_at"),
        wait_till: opt_text(r, "wait_till"),
        workflow_data: parse(opt_text(r, "workflow_data")).unwrap_or(Value::Null),
        data: parse(opt_text(r, "data")),
        wait_state: parse(opt_text(r, "wait_state")),
    }
}
...
impl Store {
    ...
    pub async fn user_count(&self) -> anyhow::Result<i64> {
        Ok(sqlx::query(&self.sql("SELECT COUNT(*) AS n FROM \"user\" WHERE password IS NOT NULL")).fetch_one(&self.pool).await?.get("n"))
    }

    pub async fn owner(&self) -> anyhow::Result<Option<User>> {
        let row = sqlx::query(&self.sql("SELECT * FROM \"user\" WHERE role = 'global:owner' LIMIT 1")).fetch_optional(&self.pool).await?;
    ...
    pub async fn create_user(&self, email: &str, first: Option<&str>, last: Option<&str>, password_hash: Option<&str>, role: &str) -> anyhow::Result<User> {
        ...
        let now = now();
        sqlx::query(&self.sql("INSERT INTO \"user\" (id, email, first_name, last_name, password, role, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"))
            .bind(&id)
    ...
    pub async fn get_user(&self, id: &str) -> anyhow::Result<Option<User>> {
        let row = sqlx::query(&self.sql("SELECT * FROM \"user\" WHERE id = ?")).bind(id).fetch_optional(&self.pool).await?;
        Ok(row.as_ref().map(user_from))
    }

    pub async fn get_user_by_email(&self, email: &str) -> anyhow::Result<Option<User>> {
        let row = sqlx::query(&self.sql("SELECT * FROM \"user\" WHERE email = ?")).bind(email.to_ascii_lowercase()).fetch_optional(&self.pool).await?;
        Ok(row.as_ref().map(user_from))
    }

    pub async fn list_users(&self) -> anyhow::Result<Vec<User>> {
        let rows = sqlx::query(&self.sql("SELECT * FROM \"user\" ORDER BY created_at, id")).fetch_all(&self.pool).await?;
    ...
    pub async fn complete_user(&self, id: &str, first: &str, last: &str, password_hash: &str) -> anyhow::Result<()> {
        sqlx::query(&self.sql("UPDATE \"user\" SET first_name = ?, last_name = ?, password = ?, updated_at = ? WHERE id = ?"))
            .bind(first)
            .bind(last)
            .bind(password_hash)
            .bind(now())
    ...
    pub async fn delete_user(&self, id: &str) -> anyhow::Result<bool> {
        let r = sqlx::query(&self.sql("DELETE FROM \"user\" WHERE id = ?")).bind(id).execute(&self.writer).await?;
        sqlx::query(&self.sql("DELETE FROM user_api_keys WHERE user_id = ?")).bind(id).execute(&self.writer).await?;
    ...
    pub async fn create_api_key(&self, user_id: &str, label: &str, hash: &str, scopes: &[String], expires_at: Option<i64>) -> anyhow::Result<ApiKey> {
        let id = new_id();
        sqlx::query(&self.sql("INSERT INTO user_api_keys (id, user_id, label, api_key_hash, scopes, created_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)"))
            .bind(&id)
            .bind(user_id)
            .bind(label)
            .bind(hash)
    ...
    fn api_key_from(r: &AnyRow) -> ApiKey {
        ApiKey {
            id: r.get("id"),
            ...
            created_at: r.get("created_at"),
            expires_at: opt_int(r, "expires_at"),
        }
    }

    pub async fn api_key_by_hash(&self, hash: &str) -> anyhow::Result<Option<ApiKey>> {
        let row = sqlx::query(&self.sql("SELECT * FROM user_api_keys WHERE api_key_hash = ?")).bind(hash).fetch_optional(&self.pool).await?;
        Ok(row.as_ref().map(Self::api_key_from))
    }

    pub async fn api_keys_of(&self, user_id: &str) -> anyhow::Result<Vec<ApiKey>> {
        let rows = sqlx::query(&self.sql("SELECT * FROM user_api_keys WHERE user_id = ? ORDER BY created_at")).bind(user_id).fetch_all(&self.pool).await?;
        Ok(rows.iter().map(Self::api_key_from).collect())
    }

    pub async fn delete_api_key(&self, user_id: &str, id: &str) -> anyhow::Result<bool> {
        let r = sqlx::query(&self.sql("DELETE FROM user_api_keys WHERE id = ? AND user_id = ?")).bind(id).bind(user_id).execute(&self.writer).await?;
        Ok(r.rows_affected() > 0)
    }

    // ---- workflows ---------------------------------------------------------

    fn workflow_from(r: &AnyRow) -> anyhow::Result<WorkflowRow> {
        Ok(WorkflowRow {
            data: serde_json::from_str(&r.get::<String, _>("data"))?,
            owner_id: opt_text(r, "owner_id"),
            project_id: opt_text(r, "project_id"),
    ...
    pub async fn workflow_row(&self, id: &str) -> anyhow::Result<Option<WorkflowRow>> {
        let row = sqlx::query(&self.sql("SELECT data, owner_id, project_id, active, created_at, updated_at FROM workflow_entity WHERE id = ?"))
            .bind(id)
    ...
    pub async fn workflow_rows(&self) -> anyhow::Result<Vec<WorkflowRow>> {
        let rows = sqlx::query(&self.sql("SELECT data, owner_id, project_id, active, created_at, updated_at FROM workflow_entity ORDER BY created_at, id"))
            .fetch_all(&self.pool)
            .await?;
    ...
    pub async fn set_workflow_owner(&self, id: &str, owner_id: Option<&str>, project_id: Option<&str>) -> anyhow::Result<()> {
        // Two statements rather than COALESCE(?, owner_id): sqlx's `Any`
        // driver types a NULL parameter as an integer on PostgreSQL.
        if let Some(owner) = owner_id {
            sqlx::query(&self.sql("UPDATE workflow_entity SET owner_id = ? WHERE id = ?")).bind(owner).bind(id).execute(&self.writer).await?;
        }
        sqlx::query(&self.sql("UPDATE workflow_entity SET project_id = ? WHERE id = ?")).bind(project_id).bind(id).execute(&self.writer).await?;
        Ok(())
    }

    pub async fn delete_workflow(&self, id: &str) -> anyhow::Result<bool> {
        let r = sqlx::query(&self.sql("DELETE FROM workflow_entity WHERE id = ?")).bind(id).execute(&self.writer).await?;
        sqlx::query(&self.sql("DELETE FROM workflows_tags WHERE workflow_id = ?")).bind(id).execute(&self.writer).await?;
        Ok(r.rows_affected() > 0)
    }
    ...
    pub async fn create_tag(&self, name: &str) -> anyhow::Result<Option<Value>> {
        let id = new_id();
        let now = now();
        let r = sqlx::query(&self.sql("INSERT INTO tag_entity (id, name, created_at, updated_at) VALUES (?, ?, ?, ?) ON CONFLICT DO NOTHING"))
            .bind(&id)
            .bind(name)
            .bind(&now)
            .bind(&now)
    ...
    pub async fn list_tags(&self) -> anyhow::Result<Vec<Value>> {
        let rows = sqlx::query(&self.sql("SELECT * FROM tag_entity ORDER BY created_at, id")).fetch_all(&self.pool).await?;
        Ok(rows.iter().map(Self::tag_json).collect())
    }

    pub async fn get_tag(&self, id: &str) -> anyhow::Result<Option<Value>> {
        let row = sqlx::query(&self.sql("SELECT * FROM tag_entity WHERE id = ?")).bind(id).fetch_optional(&self.pool).await?;
        Ok(row.as_ref().map(Self::tag_json))
    }

    pub async fn rename_tag(&self, id: &str, name: &str) -> anyhow::Result<bool> {
        let r = sqlx::query(&self.sql("UPDATE tag_entity SET name = ?, updated_at = ? WHERE id = ?")).bind(name).bind(now()).bind(id).execute(&self.writer).await?;
        Ok(r.rows_affected() > 0)
    }

    pub async fn delete_tag(&self, id: &str) -> anyhow::Result<bool> {
        let r = sqlx::query(&self.sql("DELETE FROM tag_entity WHERE id = ?")).bind(id).execute(&self.writer).await?;
        sqlx::query(&self.sql("DELETE FROM workflows_tags WHERE tag_id = ?")).bind(id).execute(&self.writer).await?;
        Ok(r.rows_affected() > 0)
    }

    pub async fn set_workflow_tags(&self, workflow_id: &str, tag_ids: &[String]) -> anyhow::Result<()> {
        sqlx::query(&self.sql("DELETE FROM workflows_tags WHERE workflow_id = ?")).bind(workflow_id).execute(&self.writer).await?;
        for t in tag_ids {
            sqlx::query(&self.sql("INSERT INTO workflows_tags (workflow_id, tag_id) VALUES (?, ?) ON CONFLICT DO NOTHING")).bind(workflow_id).bind(t).execute(&self.writer).await?;
        }
        Ok(())
    }

    pub async fn workflow_tags(&self, workflow_id: &str) -> anyhow::Result<Vec<Value>> {
        let rows = sqlx::query(&self.sql("SELECT t.* FROM tag_entity t JOIN workflows_tags wt ON wt.tag_id = t.id WHERE wt.workflow_id = ? ORDER BY t.name"))
            .bind(workflow_id)
            .fetch_all(&self.pool)
            .await?;
        Ok(rows.iter().map(Self::tag_json).collect())
    ...
    pub async fn create_variable(&self, key: &str, value: &str) -> anyhow::Result<Option<Value>> {
        let id = new_id();
        let r = sqlx::query(&self.sql("INSERT INTO variables (id, key, value, type) VALUES (?, ?, ?, 'string') ON CONFLICT DO NOTHING"))
            .bind(&id)
            .bind(key)
            .bind(value)
            .execute(&self.writer)
    ...
    pub async fn list_variables(&self) -> anyhow::Result<Vec<Value>> {
        let rows = sqlx::query(&self.sql("SELECT * FROM variables ORDER BY key")).fetch_all(&self.pool).await?;
        Ok(rows
            .iter()
    ...
    pub async fn update_variable(&self, id: &str, key: &str, value: &str) -> anyhow::Result<bool> {
        let r = sqlx::query(&self.sql("UPDATE variables SET key = ?, value = ? WHERE id = ?")).bind(key).bind(value).bind(id).execute(&self.writer).await?;
        Ok(r.rows_affected() > 0)
    }

    pub async fn delete_variable(&self, id: &str) -> anyhow::Result<bool> {
        let r = sqlx::query(&self.sql("DELETE FROM variables WHERE id = ?")).bind(id).execute(&self.writer).await?;
        Ok(r.rows_affected() > 0)
    }
    ...
    pub async fn create_project(&self, name: &str, kind: &str) -> anyhow::Result<Value> {
        let id = new_id();
        let now = now();
        sqlx::query(&self.sql("INSERT INTO project (id, name, type, created_at, updated_at) VALUES (?, ?, ?, ?, ?)"))
            .bind(&id)
            .bind(name)
            .bind(kind)
            .bind(&now)
    ...
    pub async fn list_projects(&self) -> anyhow::Result<Vec<Value>> {
        let rows = sqlx::query(&self.sql("SELECT * FROM project ORDER BY created_at")).fetch_all(&self.pool).await?;
    ...
    pub async fn project_exists(&self, id: &str) -> anyhow::Result<bool> {
        Ok(sqlx::query(&self.sql("SELECT id FROM project WHERE id = ?")).bind(id).fetch_optional(&self.pool).await?.is_some())
    }

    pub async fn update_project(&self, id: &str, name: &str) -> anyhow::Result<bool> {
        let r = sqlx::query(&self.sql("UPDATE project SET name = ?, updated_at = ? WHERE id = ?")).bind(name).bind(now()).bind(id).execute(&self.writer).await?;
        Ok(r.rows_affected() > 0)
    }

    pub async fn delete_project(&self, id: &str) -> anyhow::Result<bool> {
        let r = sqlx::query(&self.sql("DELETE FROM project WHERE id = ?")).bind(id).execute(&self.writer).await?;
        sqlx::query(&self.sql("DELETE FROM project_relation WHERE project_id = ?")).bind(id).execute(&self.writer).await?;
        Ok(r.rows_affected() > 0)
    }

    pub async fn add_project_relation(&self, project_id: &str, user_id: &str, role: &str) -> anyhow::Result<()> {
        sqlx::query(&self.sql("INSERT INTO project_relation (project_id, user_id, role) VALUES (?, ?, ?) ON CONFLICT (project_id, user_id) DO UPDATE SET role = excluded.role"))
            .bind(project_id)
            .bind(user_id)
            .bind(role)
            .execute(&self.writer)
    ...
    pub async fn set_credential_owner(&self, id: &str, owner_id: &str) -> anyhow::Result<()> {
        sqlx::query(&self.sql("UPDATE credentials_entity SET owner_id = ? WHERE id = ?")).bind(owner_id).bind(id).execute(&self.writer).await?;
        Ok(())
    }

    pub async fn credential_owner(&self, id: &str) -> anyhow::Result<Option<String>> {
        let row = sqlx::query(&self.sql("SELECT owner_id FROM credentials_entity WHERE id = ?")).bind(id).fetch_optional(&self.pool).await?;
        Ok(row.and_then(|r| opt_text(&r, "owner_id")))
    }

    pub async fn delete_credential(&self, id: &str) -> anyhow::Result<bool> {
        let r = sqlx::query(&self.sql("DELETE FROM credentials_entity WHERE id = ?")).bind(id).execute(&self.writer).await?;
        Ok(r.rows_affected() > 0)
    }
    ...
    async fn batched(&self, sql: &'static str, binds: Vec<Bind>) -> anyhow::Result<(u64, i64)> {
        ...
        let returning = sql.trim_start().starts_with("INSERT INTO execution_entity");
        self.batch.send(Op { sql: self.sql(sql).into_owned(), returning, binds, reply }).map_err(|_| anyhow::anyhow!("the database writer has stopped"))?;
    ...
    pub async fn get_execution(&self, id: i64) -> anyhow::Result<Option<ExecutionRow>> {
        let row = sqlx::query(&self.sql(&format!("SELECT {EXEC_COLS} FROM execution_entity WHERE id = ?"))).bind(id).fetch_optional(&self.pool).await?;
    ...
    pub async fn list_executions(&self, f: &ExecutionFilter) -> anyhow::Result<Vec<ExecutionRow>> {
        ...
        let sql = self.sql(&sql).into_owned();
    ...
    pub async fn mark_crashed(&self) -> anyhow::Result<u64> {
        let r = sqlx::query(&self.sql("UPDATE execution_entity SET status = 'crashed', stopped_at = ? WHERE status IN ('running', 'new')"))
            .bind(now())
            .execute(&self.writer)
            .await?;
        Ok(r.rows_affected())
    ...
    pub async fn due_waiting(&self, now_iso: &str) -> anyhow::Result<Vec<i64>> {
        let rows = sqlx::query(&self.sql("SELECT id FROM execution_entity WHERE status = 'waiting' AND wait_till IS NOT NULL AND wait_till <= ?"))
            .bind(now_iso)
            .fetch_all(&self.pool)
            .await?;
    ...
    pub async fn prune_executions(&self, older_than_iso: &str) -> anyhow::Result<u64> {
        let r = sqlx::query(&self.sql("DELETE FROM execution_entity WHERE stopped_at IS NOT NULL AND stopped_at < ? AND status NOT IN ('running', 'waiting', 'new')"))
            .bind(older_than_iso)
            .execute(&self.writer)
            .await?;
        Ok(r.rows_affected())
    ...
    pub async fn active_workflow_count(&self) -> anyhow::Result<i64> {
        Ok(sqlx::query(&self.sql("SELECT COUNT(*) AS n FROM workflow_entity WHERE active = 1")).fetch_one(&self.pool).await?.get("n"))
    }
}
